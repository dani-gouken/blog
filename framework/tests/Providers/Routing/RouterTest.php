...
class RouterTest extends BasicTest{
    ...
    public function testItMatchRoute(){
        ...
        $this->assertEquals($matchedRoute->route,$route);
    ...
    public function testItMatchRouteWithValidParameters(){
        ...
        $matchedRoute = $router->dispatch($request);
        $this->assertEquals($matchedRoute->route,$route);
        $this->assertEquals("aze-aze",$matchedRoute->arguments["name"]);
        $this->assertEquals("1",$matchedRoute->arguments["id"]);
    ...
    public function testItMatchRouteInsideRouteGroup(){
        ...
        $matchedRoute = $router->dispatch($request);
        $this->assertEquals($matchedRoute->route->getName(),$route->getName());
        $this->assertEquals($matchedRoute->route->getRouteGroup()->getPattern(),"/baz");
        $request = $this->createRequest("foo/bar","GET");
        $matchedRoute = $router->dispatch($request);
        $this->assertEquals($matchedRoute->route->getName(),$route2->getName());
        $this->assertEquals($matchedRoute->route->getRouteGroup()->getPattern(),"foo/");
    }
